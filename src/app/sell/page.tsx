...
import { Camera, Image as ImageIcon, X, Trash, ArrowRight, Save, Info, MapPin } from 'lucide-react';
...
export default function SellPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/items/${responseData.item_id}`);
    } catch (err: any) {
      setErrorLine(err.message || 'An error occurred while posting your listing.');
      ...
      <header className="sticky top-0 z-50 bg-[#F9F9F9]/80 backdrop-blur-[20px] pb-3 border-b border-[#E2E2E2]">
        <div className="max-w-7xl mx-auto px-4 pt-4 flex items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-4">
             <Link href="/" className="text-sm font-bold text-[#5f5e5e] hover:text-[#1A1C1C] transition-colors">Cancel</Link>
          </div>
        </div>
      </header>

      <main className="max-w-3xl mx-auto w-full px-4 py-8">
        <h1 className="text-3xl font-extrabold text-[#1A1C1C] mb-8">Post an Ad</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-white p-6 rounded-[24px] border border-[#EEEEEE] shadow-sm space-y-6">
            ...
            <div>
              ...
              <input 
                ...
                placeholder="Key feature of your item (e.g. brand, model, age)"
                maxLength={70}
                required
          ...
          <div className="bg-white p-6 rounded-[24px] border border-[#EEEEEE] shadow-sm space-y-4">
            ...
            <p className="text-xs text-[#5f5e5e] font-medium leading-relaxed">
              For now, you can upload 1 main picture for your ad. Make sure the item is clear and well-lit.
            </p>
              ...
              <div className="relative w-32 h-32 rounded-2xl overflow-hidden border border-[#E2E2E2] group">
               ...
               <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-[#E2E2E2] bg-[#F9F9F9] hover:bg-[#F2F2F2] transition-colors rounded-2xl cursor-pointer">
                 <div className="flex flex-col items-center justify-center pt-5 pb-6">
                   ...
                   <p className="mb-1 text-sm font-bold text-[#1A1C1C]">Click to upload photo</p>
                   <p className="text-xs text-[#5f5e5e] font-medium">PNG, JPG up to 5MB</p>
                 </div>
          ...
          {/* Card 4: Confirm Location (Auto) */}
          <div className="bg-white p-6 rounded-[24px] border border-[#EEEEEE] shadow-sm space-y-4">
            <h2 className="text-lg font-bold text-[#1A1C1C] border-b border-[#EEEEEE] pb-4">Confirm your Location</h2>
            
            <div className="flex items-start gap-4 p-4 rounded-xl bg-[#EEFCF4] border border-[#d1fae5]">
               <div className="mt-0.5 w-10 h-10 bg-[#006E17]/10 text-[#006E17] rounded-full flex items-center justify-center shrink-0">
                  <MapPin className="w-5 h-5" />
               </div>
               <div>
                  <h4 className="font-bold text-[#006E17]">Based on your verified account</h4>
                  <p className="text-sm font-medium text-[#006E17]/80 mt-1">
                    Your ad will automatically be grouped and shown to peers from your registered college.
                  </p>
               </div>
            </div>
          </div>
